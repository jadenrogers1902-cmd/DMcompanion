...
export function CreateAdventureButton({ campaignId, size = 'sm' }: CreateAdventureButtonProps) {
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    setError(null)
    const result = await createAdventure(campaignId, { title, description })
    if (result?.error) {
      setError(result.error)
      setSaving(false)
      return
    }
    router.push(`/campaigns/${campaignId}/adventures/${result.adventureId}`)
  }
        ...
        >
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-zinc-100">New Adventure</h2>
