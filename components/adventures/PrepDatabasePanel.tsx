  ...
  return (
    <section className="rounded-xl border border-border bg-panel p-4">
      ...
      {(pinnedNotes.length > 0 || pinnedLinks.length > 0) && (
        <div className="mb-4 rounded-lg border border-accent/50 bg-accent/20 p-3">
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-accent">
            Pinned
          </h3>
          <div className="flex flex-col gap-1.5 text-sm">
            {pinnedNotes.map((note) => (
              <div key={note.id} className="text-muted">
            ...
            {pinnedLinks.map((link) => (
              <a
                ...
                className="truncate text-accent hover:text-accent"
              >
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div>
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-faint">
              Notes <span className="text-faint">({notes.length})</span>
            </h3>
          </div>
          {notes.length === 0 ? (
            <p className="text-xs text-faint">No prep notes yet.</p>
          ) : (
              ...
              {notes.map((note, index) => (
                <div key={note.id} className="rounded-lg border border-border bg-canvas p-3">
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
                    ...
                    />
                    <label className="flex items-center gap-2 rounded-lg border border-border px-2 text-xs text-muted">
                      <input
                        type="checkbox"
                        checked={note.pinned}
                    ...
                    <button
                      ...
                      className="rounded-lg border border-border px-2 text-xs text-faint hover:border-red-800 hover:text-red-300"
                    >
                      Delete
                    </button>
                  </div>
        ...
        <div>
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-faint">
            Important Links <span className="text-faint">({links.length})</span>
          </h3>
          {links.length === 0 ? (
            <p className="text-xs text-faint">No important links yet.</p>
          ) : (
            <div className="flex flex-col gap-3">
              {links.map((link, index) => (
                <div key={link.id} className="rounded-lg border border-border bg-canvas p-3">
                  <div className="mb-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_9rem]">
                    <Input
                  ...
                  <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="rounded-lg border border-border px-2 py-1 text-xs text-faint hover:border-red-800 hover:text-red-300"
                      >
                        Delete
                      </button>
                    </div>
