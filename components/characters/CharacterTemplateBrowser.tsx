...
function Section({
  ...
  return (
    <details open={open} className="rounded-lg border border-border bg-shell">
      <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-content marker:text-accent">
        {title}
      </summary>
...
function SimpleList({ items }: { items?: string[] }) {
  if (!items || items.length === 0) return <p className="text-sm text-faint">None listed.</p>
  return (
...
function JsonBlock({ value }: { value: unknown }) {
  return (
    <pre className="max-h-96 overflow-auto rounded-lg border border-border bg-panel p-3 text-xs leading-relaxed text-muted">
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
        <div className="rounded-lg border border-border bg-panel p-3">
          <p className="text-sm font-semibold text-content">{spellcasting.ability}</p>
          <p className="text-xs text-faint">Spellcasting ability</p>
        </div>
        <div className="rounded-lg border border-border bg-panel p-3">
          <p className="text-sm font-semibold text-content">{spellcasting.spellSaveDC}</p>
          <p className="text-xs text-faint">Save DC</p>
        </div>
        <div className="rounded-lg border border-border bg-panel p-3">
          <p className="text-sm font-semibold text-content">+{spellcasting.spellAttackBonus}</p>
          <p className="text-xs text-faint">Spell attack</p>
        </div>
      </div>
      <div>
        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-faint">Cantrips</h4>
        <SimpleList items={spellcasting.cantripsKnown} />
      </div>
      {spellcasting.spellbook && (
        <div>
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-faint">Spellbook</h4>
          <SimpleList items={spellcasting.spellbook} />
        </div>
      )}
      {spellcasting.alwaysPreparedDomainSpells && (
        <div>
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-faint">Always Prepared / Domain</h4>
          <JsonBlock value={spellcasting.alwaysPreparedDomainSpells} />
        </div>
      )}
      {spellcasting.preparedSpellsRule && (
        <p className="text-sm text-muted">{spellcasting.preparedSpellsRule}</p>
      )}
    </div>
  )
}
        ...
        <form action={finalize} className="flex flex-col gap-5">
          ...
          {spellcasting && (
            <div className="rounded-lg border border-border bg-shell p-4">
              <h3 className="text-sm font-semibold text-content">Prepared Spells</h3>
              {spellcasting.preparedSpellsRule && (
              ...
              {spellOptions.length > 0 && (
                <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {spellOptions.map((spell) => (
                    <label key={spell} className="flex items-center gap-2 rounded-md border border-border bg-panel px-3 py-2 text-sm text-muted">
