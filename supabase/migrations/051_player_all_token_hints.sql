-- Generalize player-safe live-map token hints from portals to every
-- discoverable hidden token. The hint row preserves only map placement data and
-- a generic marker; names, notes, destination links, health, actions, and other
-- gameplay details stay hidden until the token is discovered.
...
AS $$
  SELECT
    ...
    CASE
      ...
      ELSE 'Unrevealed token'
    END AS name,
    t.x,
    t.y,
    t.size,
    CASE WHEN t.visible_to_players THEN COALESCE(t.color, '#6b7280') ELSE '#a855f7' END AS color,
