-- Allow player-confirmed combat movement when the DM has unlocked movement.
-- Travel/freeroam movement stays immediate; combat movement uses the round
-- anchor (last_x/last_y) and movement_used so the confirm UI has an
